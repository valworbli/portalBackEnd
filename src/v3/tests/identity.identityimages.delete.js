...
const defUser = new Users({
  ...
  verify_token: 'cc2b039697793f4f38aa908f07fd2971',
});
...
describe('## User', function() {
  this.timeout(30000);
  const testUrl = baseTestUrl + 'identityimages/';
  let jwtToken = '';
  ...
  describe(`# DELETE ${testUrl}`, () => {
    ...
    it('deletes a non-existing image - should return 200 and data true and completed false', (done) => {
      request(app)
          ...
          .then((res) => {
            ...
            assert(res.body.missingDocuments.length === 2, 'Err missingDocuments is an empty array');
            done();
          })
          .catch(done);
    });

    it('uploads identity images - should return 200, data true and completed false', (done) => {
      request(app)
          .post(baseTestUrl + 'image/')
          .set('Accept', 'application/json')
          .set('Authorization', `Bearer ${jwtToken}`)
          ...
          .then((res) => {
            ...
            assert(res.body.missingDocuments[0] === 'selfie', 'Err missingDocuments is not \'selfie\'');
            done();
          })
          .catch(done);
    });

    it('deletes all the images - should return 200 and data true and completed false', (done) => {
      request(app)
          .delete(testUrl)
          .set('Accept', 'application/json')
          .set('Authorization', `Bearer ${jwtToken}`)
    ...
    it('uploads all the images - should return 200 and data true', (done) => {
      request(app)
          .post(baseTestUrl + 'image/')
          .set('Accept', 'application/json')
          .set('Authorization', `Bearer ${jwtToken}`)
    ...
    it('deletes the identity images - should return 200 and data true and completed false', (done) => {
      request(app)
          .delete(testUrl)
          .set('Accept', 'application/json')
          .set('Authorization', `Bearer ${jwtToken}`)
